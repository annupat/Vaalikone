...
<body>
	...
	<c:forEach var="kysymys" items="<%=readAllAdminQuestions()%>">
		<li>${kysymys}<a
			href='/rest/questionservice/deleteadminquestion/${kysymys.kysymysId}'>Delete</a>
			<a
			href='/rest/questionservice/readtoupdatekysymys/${kysymys.kysymysId}'>Update</a>
	</c:forEach>

	<h3>Uusien kysymysten lisääminen</h3>
	<form action='/addkysymys' method='post'>
		<input type='text' name='kysymys' value=''> <input
			type='submit' name='ok' value='Lisää uusi kysymys'>
	</form>
